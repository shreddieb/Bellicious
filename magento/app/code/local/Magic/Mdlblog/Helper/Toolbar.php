...
class Magic_Mdlblog_Helper_Toolbar extends Mage_Core_Block_Abstract
{
    public function create($block, $params = array())
    {
        ...
        $block->setChild('magic_mdlblog_comments_toolbar', $toolbar);
    }
    ...
    {
        $block = $this->getToolbarParentBlock()->getLayout()->getBlock('magic_mdlblog_list_toolbar');
        if (!$block) {
            return $this->getToolbarParentBlock()->getLayout()->createBlock('mdlblog/product_toolbar', microtime());
        }
        return $block;
    }
}