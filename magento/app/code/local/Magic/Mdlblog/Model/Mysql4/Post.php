...
class Magic_Mdlblog_Model_Mysql4_Post extends Mage_Core_Model_Mysql4_Abstract
{
    protected function _construct()
    {
        $this->_init('mdlblog/post', 'post_id');
    }
    ...
    protected function _beforeSave(Mage_Core_Model_Abstract $object)
    {
        if (!$this->getIsUniqueIdentifier($object)) {
            Mage::throwException(Mage::helper('mdlblog')->__('Post Identifier already exist.'));
        }

        if ($this->isNumericIdentifier($object)) {
            Mage::throwException(Mage::helper('mdlblog')->__('Post Identifier cannot consist only of numbers.'));
        }
    ...
    public function getCategoriesForPost($posts)
    {
        $select = $this->_getReadAdapter()
            ->select()
            ->from(array('post_category' => $this->getTable('mdlblog/post_cat')))
            ->joinLeft(
                array('category_store' => $this->getTable('mdlblog/cat_store')),
                'post_category.cat_id = category_store.cat_id', array()
            )
            ->joinLeft(
                array('category_main' => $this->getTable('mdlblog/cat')), 'post_category.cat_id = category_main.cat_id',
