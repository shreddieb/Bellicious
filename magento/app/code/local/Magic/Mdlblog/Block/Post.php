...
class Magic_Mdlblog_Block_Post extends Magic_Mdlblog_Block_Abstract
{
    public function getPost()
    {
        if (!$this->hasData('post')) {
            if ($this->getPostId()) {
                $post = Mage::getModel('mdlblog/post')->load($this->getPostId());
            } else {
                $post = Mage::getSingleton('mdlblog/post');
            }
            $category = Mage::getSingleton('mdlblog/cat')->load(
    ...
    public function getBookmarkHtml($post)
    {
        if ($this->_helper()->isBookmarksPost()) {
            return $this->setTemplate('magic_mdlblog/bookmark.phtml')->setPost($post)->renderView();
        }
    }
    ...
    public function getComment()
    {
        if (!$this->hasData('commentCollection')) {
            $collection = Mage::getModel('mdlblog/comment')
                ->getCollection()
                ->addPostFilter($this->getPost()->getPostId())
                ->setOrder('created_time', 'DESC')
                ->addApproveFilter(2)
            ;
            $collection->setPageSize((int)Mage::helper('mdlblog')->commentsPerPage());
    ...
    {
        return Mage::getStoreConfig('mdlblog/comments/enabled');
    }

    public function getLoginRequired()
    {
        return Mage::getStoreConfig('mdlblog/comments/login');
    }
    ...
    {
        Mage::helper('mdlblog/toolbar')->create(
            $this,
            array(
    ...
    protected function getCategory()
    {
        if (!$this->hasData('postCategory')) {
            $this->setData(
                'postCategory', Mage::getSingleton('mdlblog/cat')->load($this->getRequest()->getParam('cat'), "identifier")
            );
        }
