<?php
class Mdl_Ajaxnav_Model_Layer_Filter_Category extends Mage_Catalog_Model_Layer_Filter_Category {

    protected $cat = null;

    public function apply(Zend_Controller_Request_Abstract $request, $filterBlock) {
        // very small optimization
        $catId = (int) Mage::helper('ajaxnav')->getParam($this->getRequestVar());
        if ($catId) {
    ...
    protected function _getItemsData() {
        ...
        $key .= Mage::helper('ajaxnav')->getCacheKey('cat');
        $pageKey = Mage::getBlockSingleton('page/html_pager')->getPageVarName();
        $queryStr = Mage::helper('ajaxnav')->getParams(true, $pageKey);
        $data = $this->getLayer()->getAggregator()->getCacheData($key);

        if ($data === null) {
            $category = null;
            $showTopCategories = Mage::getStoreConfig('ajaxnav/ajaxnav/top_cats');
            ...
            if (Mage::getStoreConfig('ajaxnav/ajaxnav/reset_filters')) {
                $queryStr = '';
            }
